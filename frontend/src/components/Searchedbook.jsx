...
function Blog() {
    ...
    const getData = async () => {
        fetch("http://localhost:9000/bookbits/" + search)
        .then((res) => res.json())
        .then((json) => {
            setState(json)
    })
    ...
    const toPost=(element)=>{
        navigate('/postbook',{state:{title: element.title, content: element.content, category: element.category}});
    }
